...
public class Perceptron {
		...
		public static void main(String[] args) {
			...
			margin = Integer.parseInt(args[2]);
			halting = Boolean.getBoolean(args[3]);
			...
			mistakes = 0;
			T = 1;
			perceptron(corpus, halting, true);
		...
		public static void perceptron(double[][] S, boolean halting, boolean printMistakes) {
			// w[] <- 0, b <- 0, t <- 0
			b = 0;
			t = 0;
			...
			while (t < T && haltingCount <= 1000) {
				...
				for (double[] x : S) { //x[0] is label
					...
					instanceCount++;
					if (printMistakes && (instanceCount == 10000 || instanceCount == 20000 || instanceCount == 30000 || instanceCount == 40000 
							|| instanceCount == 50000)) {
						System.out.println("Mistakes at " + instanceCount + " instances: " + mistakes);
					}
				} // end for
				if (!halting) {
					t = t + 1;
		...
		public static void testEtaChoices(double[] etaChoices) {
			for (double etaChoice : etaChoices) {
				...
				eta = etaChoice;
				perceptron(d1, false, false);
				System.out.println(b);
				double accuracy = getAccuracy(d2);
		...
		public static double getAccuracy(double[][] d2) {
			double correct = 0.0;
			double total = 5000.0;
